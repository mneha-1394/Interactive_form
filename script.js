...
function massValidate(){
    ...
    function validateCard(){
        let cardNumError=document.querySelector(".card-num-error")
        if(cardNumber.value.length > 0 && cardNumber.value.length<16){
            cardNumError.innerHTML="Wrong format!"
        } else if(cardNumber.value=="") {
            cardNumError.innerHTML="Can't be blank! Abeg Fill it!!!!"
        } else{
    ...
    function validateExpiry(){
        ...
        let expYear=/^[0-9][0-2]{4}$/
         if(expiry[0].value.match(expMonth)){
            ...
            expiryErrorMsg.innerHTML="Can't be Blank! Abeg Fill!!!"
         } else{
    ...
    function validateCvc(){
        ...
        let cvcExp=/^[0-9]{3}$/
        if(cvc.value===""){
            cvcErrorMsg.innerHTML="Can not Be Blank! Abeg fill!"
